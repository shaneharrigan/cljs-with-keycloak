shadow$provide.module$node_modules$hash_DOT_js$lib$hash$utils = function(global, require, module, exports) {
  function htonl(w) {
  ...
  var assert = require("module$node_modules$minimalistic_assert$index");
  global = require("module$node_modules$inherits$inherits_browser");
  exports.inherits = global;
  exports.toArray = function(msg, enc) {
    ...
    if ("string" === typeof msg) {
      if (!enc) {
        for (var i = enc = 0; i < msg.length; i++) {
          ...
          if (128 > c) {
            ...
            if (2048 > c) {
              ...
              var JSCompiler_inline_result = msg;
              var i$jscomp$0 = i;
              JSCompiler_inline_result = 55296 !== (JSCompiler_inline_result.charCodeAt(i$jscomp$0) & 64512) ? !1 : 0 > i$jscomp$0 || i$jscomp$0 + 1 >= JSCompiler_inline_result.length ? !1 : 56320 === (JSCompiler_inline_result.charCodeAt(i$jscomp$0 + 1) & 64512);
              JSCompiler_inline_result ? (c = 65536 + ((c & 1023) << 10) + (msg.charCodeAt(++i) & 1023), res[enc++] = c >> 18 | 240, res[enc++] = c >> 12 & 63 | 128) : res[enc++] = c >> 12 | 224;
              res[enc++] = c >> 6 & 63 | 128;
            }
            res[enc++] = c & 63 | 128;
          }
        ...
        for (msg = msg.replace(/[^a-z0-9]+/ig, ""), 0 !== msg.length % 2 && (msg = "0" + msg), i = 0; i < msg.length; i += 2) {
  ...
  exports.join32 = function(msg, start, end, endian) {
    ...
    for (var i = 0; i < end.length; i++, start += 4) {
      end[i] = ("big" === endian ? msg[start] << 24 | msg[start + 1] << 16 | msg[start + 2] << 8 | msg[start + 3] : msg[start + 3] << 24 | msg[start + 2] << 16 | msg[start + 1] << 8 | msg[start]) >>> 0;
    }
    return end;
  };
  exports.split32 = function(msg, endian) {
    for (var res = Array(4 * msg.length), i = 0, k = 0; i < msg.length; i++, k += 4) {
      var m = msg[i];
      "big" === endian ? (res[k] = m >>> 24, res[k + 1] = m >>> 16 & 255, res[k + 2] = m >>> 8 & 255, res[k + 3] = m & 255) : (res[k + 3] = m >>> 24, res[k + 2] = m >>> 16 & 255, res[k + 1] = m >>> 8 & 255, res[k] = m & 255);
    }
    return res;
  };
  ...
  exports.sum64 = function(buf, pos, ah, al) {
    ...
    buf[pos] = (lo < al ? 1 : 0) + ah + buf[pos] >>> 0;
    buf[pos + 1] = lo;
  };
  ...
  exports.sum64_4_hi = function(ah, al, bh, bl, ch, cl, dh, dl) {
    ...
    bl = bl + cl >>> 0;
    al += bl < cl ? 1 : 0;
    return ah + bh + ch + dh + (al + (bl + dl >>> 0 < dl ? 1 : 0)) >>> 0;
  };
  ...
  exports.sum64_5_hi = function(ah, al, bh, bl, ch, cl, dh, dl, eh, el) {
    ...
    bl = bl + dl >>> 0;
    al += bl < dl ? 1 : 0;
    return ah + bh + ch + dh + eh + (al + (bl + el >>> 0 < el ? 1 : 0)) >>> 0;
  };
