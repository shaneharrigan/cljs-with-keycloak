shadow$provide.module$node_modules$asn1_DOT_js$lib$asn1$base$node = function(global, require, module, exports) {
  ...
  var Reporter = require("module$node_modules$asn1_DOT_js$lib$asn1$base$index").Reporter, EncoderBuffer = require("module$node_modules$asn1_DOT_js$lib$asn1$base$index").EncoderBuffer, DecoderBuffer = require("module$node_modules$asn1_DOT_js$lib$asn1$base$index").DecoderBuffer, assert = require("module$node_modules$minimalistic_assert$index");
  global = "seq seqof set setof objid bool gentime utctime null_ enum int objDesc bitstr bmpstr charstr genstr graphstr ia5str iso646str numstr octstr printstr t61str unistr utf8str videostr".split(" ");
  var methods = "key obj use optional explicit implicit def choice any contains".split(" ").concat(global);
  module.exports = Node;
  var stateProps = "enc parent children tag args reverseArgs choice optional any obj use alteredUse key default explicit implicit contains".split(" ");
  ...
  Node.prototype._useArgs = function(args) {
    ...
    0 !== children.length && (assert(null === state.children), state.children = children, children.forEach(function(child) {
      child._baseState.parent = this;
    }, this));
    0 !== args.length && (assert(null === state.args), state.args = args, state.reverseArgs = args.map(function(arg) {
  ...
  "_peekTag _decodeTag _use _decodeStr _decodeObjid _decodeTime _decodeNull _decodeInt _decodeBool _decodeList _encodeComposite _encodeStr _encodeObjid _encodeTime _encodeNull _encodeInt _encodeBool".split(" ").forEach(function(method) {
    Node.prototype[method] = function() {
      throw Error(method + " not implemented for encoding: " + this._baseState.enc);
    };
  });
  global.forEach(function(tag) {
  ...
  Node.prototype._decode = function(input, options) {
    ...
    if (present) {
      if (null !== state.explicit) {
        tag = this._decodeTag(input, state.explicit);
        if (input.isError(tag)) {
          return tag;
        }
        input = tag;
      }
      ...
      options && options.track && null !== state.tag && options.track(input.path(), tag, input.length, "tagged");
      ...
      state.any || null !== state.choice || null === state.children || state.children.forEach(function(child) {
        child._decode(input, options);
      });
      !state.contains || "octstr" !== state.tag && "bitstr" !== state.tag || (result = new DecoderBuffer(result), result = this._getUse(state.contains, input._reporterState.obj)._decode(result, options));
    }
    state.obj && present && (result = input.leaveObject(prevObj));
    null === state.key || null === result && !0 !== present ? null !== prevKey && input.exitKey(prevKey) : input.leaveKey(prevKey, state.key, result);
    return result;
  };
  Node.prototype._decodeGeneric = function(tag, input, options) {
    var state = this._baseState;
    return "seq" === tag || "set" === tag ? null : "seqof" === tag || "setof" === tag ? this._decodeList(input, tag, state.args[0], options) : /str$/.test(tag) ? this._decodeStr(input, tag, options) : "objid" === tag && state.args ? this._decodeObjid(input, state.args[0], state.args[1], options) : "objid" === tag ? this._decodeObjid(input, null, null, options) : "gentime" === tag || "utctime" === tag ? this._decodeTime(input, tag, options) : "null_" === tag ? this._decodeNull(input, options) : "bool" === 
    tag ? this._decodeBool(input, options) : "objDesc" === tag ? this._decodeStr(input, tag, options) : "int" === tag || "enum" === tag ? this._decodeInt(input, state.args && state.args[0], options) : null !== state.use ? this._getUse(state.use, input._reporterState.obj)._decode(input, options) : input.error("unknown tag: " + tag);
  };
  ...
  Node.prototype._decodeChoice = function(input, options) {
    ...
    return match ? result : input.error("Choice not matched");
  };
  ...
  Node.prototype._encode = function(data, reporter, parent) {
    ...
    if (null === state["default"] || state["default"] !== data) {
      if (data = this._encodeValue(data, reporter, parent), void 0 !== data && !this._skipDefault(data, reporter, parent)) {
        return data;
      }
    }
  };
  Node.prototype._encodeValue = function(data, reporter, parent) {
    ...
    if (state.any) {
      ...
      content = state.children.map(function(child) {
        ...
        child = child._encode(data[child._baseState.key], reporter, data);
      ...
      if ("seqof" === state.tag || "setof" === state.tag) {
        ...
        content = this.clone();
        content._baseState.implicit = null;
        content = this._createEncoderBuffer(data.map(function(item) {
          return this._getUse(this._baseState.args[0], data)._encode(item, reporter);
        }, content));
      } else {
        null !== state.use ? result = this._getUse(state.use, parent)._encode(data, reporter) : (content = this._encodePrimitive(state.tag, data), primitive = !0);
      }
    }
    if (!state.any && null === state.choice) {
      parent = null !== state.implicit ? state.implicit : state.tag;
      var cls = null === state.implicit ? "universal" : "context";
      null === parent ? null === state.use && reporter.error("Tag could be omitted only for .use()") : null === state.use && (result = this._encodeComposite(parent, primitive, cls, content));
    }
    null !== state.explicit && (result = this._encodeComposite(state.explicit, !1, "context", result));
    return result;
  };
