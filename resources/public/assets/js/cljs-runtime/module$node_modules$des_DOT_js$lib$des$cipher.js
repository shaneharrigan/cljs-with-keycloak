shadow$provide.module$node_modules$des_DOT_js$lib$des$cipher = function(global, require, module, exports) {
  ...
  Cipher.prototype._updateEncrypt = function(data) {
    var inputOff = 0, outputOff = 0, out = Array(((this.bufferOff + data.length) / this.blockSize | 0) * this.blockSize);
  ...
  Cipher.prototype._updateDecrypt = function(data) {
    for (var inputOff = 0, outputOff = 0, count = Math.ceil((this.bufferOff + data.length) / this.blockSize) - 1, out = Array(count * this.blockSize); 0 < count; count--) {
      inputOff += this._buffer(data, inputOff), outputOff += this._flushBuffer(out, outputOff);
    }
    this._buffer(data, inputOff);
    return out;
  };
  Cipher.prototype.final = function(buffer) {
    var first;
    buffer && (first = this.update(buffer));
    buffer = "encrypt" === this.type ? this._finalEncrypt() : this._finalDecrypt();
    return first ? first.concat(buffer) : buffer;
  };
