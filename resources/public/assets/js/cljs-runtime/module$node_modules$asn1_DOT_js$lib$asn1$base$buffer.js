shadow$provide.module$node_modules$asn1_DOT_js$lib$asn1$base$buffer = function(global, require, module, exports) {
  function DecoderBuffer(base, options) {
    Reporter.call(this, options);
    Buffer.isBuffer(base) ? (this.base = base, this.offset = 0, this.length = base.length) : this.error("Input not Buffer");
  }
  ...
  var Reporter = require("module$node_modules$asn1_DOT_js$lib$asn1$base$index").Reporter, Buffer = require("module$node_modules$buffer$index").Buffer;
  global(DecoderBuffer, Reporter);
  ...
  DecoderBuffer.prototype.skip = function(bytes, fail) {
    ...
    fail = new DecoderBuffer(this.base);
    fail._reporterState = this._reporterState;
    fail.offset = this.offset;
    fail.length = this.offset + bytes;
    this.offset += bytes;
    return fail;
  };
  ...
  EncoderBuffer.prototype.join = function(out, offset) {
    ...
    Array.isArray(this.value) ? this.value.forEach(function(item) {
      item.join(out, offset);
      offset += item.length;
    }) : ("number" === typeof this.value ? out[offset] = this.value : "string" === typeof this.value ? out.write(this.value, offset) : Buffer.isBuffer(this.value) && this.value.copy(out, offset), offset += this.length);
    return out;
  };
};
